const countOccurence = (array, candidate) => {
  const count = array
    .flat()
    .reduce((acc, current) => { return current === candidate ? ++acc : acc }, 0);
  return count;
}
...
const reverse = (array) => {
  const reversed = array.reduce((acc, current) => { acc.unshift(current); return acc }, []);
  return reversed;
}

const frequency = (array) => {
  const uniqueWords = array.flat().filter((item, index, arr) => arr.indexOf(item) === index);
  const frequencySummary = uniqueWords.flatMap(x => `${x}:${countOccurence(array, x)}`);
...
const movies = [["horror", "action"], ["action"], ["drama", "action"], ["science fiction"]];
...
const iceCreamFlavors = [["vanilla", "chocolate"], ["strawberry"], ["chocolate"]];
...
const poemDraft = ['Rain falls rain sings',
...
const participants = [["task1", "task2"], ["task1", "task4"], ["task3"], ["task4"]];
...
const subjects = [["hindi", "english", "math"], ["biology", "physices", "chemistry"]];
...
const colors = [["red", "black", "white"], ["black", "white"], ["red", "brown"], ["white"]];
...
const journal = ['rain falls rain sings',
  'Night hums night brings dreams',
  'rain falls softly hearts awake',
  'Night listens dreams remake'];
...
const poetryFragments = [["rain", "falls", "rain", "sings"], ["Night", "hums", "night", "brings", "dreams"],
["rain", "falls", "softly", "hearts", "awake"], ["Night", "listens", "dreams", "remake"]];

console.log(poetryFragments.flatMap(x => x.join(' ')));

// List unique cartoon characters favored by children.
const partipents = [["Tom", "Jerry"], ["Jerry", "doremon"], ["nobita", "doremon"]];
...
const shirtsColor = [["red", "black", "white"], ["black", "white"], ["red", "brown"], ["white"]];
console.log(countOccurence(shirtsColor, "white"));

// Combine all notes written during a meeting into one list.
const notes = [["red", "black", "white"], ["black", "white"], ["red", "brown"], ["white"]];
...
const isLowerCase = letters.every(x => { return x.toLowerCase() === x });
